...
import org.authentication.ambientaudio.reedsolomon.GenericGF;
import org.authentication.ambientaudio.reedsolomon.ReedSolomonDecoder;
import org.authentication.ambientaudio.reedsolomon.ReedSolomonEncoder;
import org.authentication.ambientaudio.reedsolomon.ReedSolomonException;
...
public class ECCoder {
	...
	public int[] calculateSharedKey(byte[][] fingerprint) {
		if (fingerprint == null)
			return null; // Error: fingerprint not calculated
		...
		// change the array of bytes to array of ints
		int[] code_word = new int[n];
		for (int i=0; i<n; i++)
			code_word[i] = (int) codewordBytes[i];
